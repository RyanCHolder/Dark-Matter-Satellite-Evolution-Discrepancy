...
void GravAccel_DynamicNFW(void);
void GravAccel_TriaxialNFW(void);
...
void add_analytic_gravitational_forces()
{
    ...
    //GravAccel_DynamicNFW();               // Mass changing over time NFW (based on TNG halo values)
    //GravAccel_TriaxialNFW();              // Nonspherical NFW profile
...
{
    double NFW_M200=1000, NFW_C=4; /* NFW mass inside R200 (in code units), and concentration =R200/Rs */
...
/* time evolving NFW potential */
void GravAccel_DynamicNFW()
{
    //double NFW_M200=1000, NFW_C=4; /* NFW mass inside R200 (in code units), and concentration =R200/Rs */
    
    int n_table = 14; // number of table entries below (must match!)
    // scale factor for cosmological runs (must be in monotonic increasing order!)
    // double t_table[14] = {0.2, 0.250, 0.266, 0.285, 0.308, 0.333, 0.363, 0.400, 0.444, 0.500, 0.572, 0.667, 0.800, 1.000};
    
    // time values for snapshots after peak mass of tree id 101  (in code units, Gyr)
    double t_table[44] = {0.0, 0.12958366998552062, 0.30548364412161266, 0.4397516266271104, 0.6219020863931242, 0.7608614848235291, 0.949271901426024, 1.092929819186259, 1.2386035625995566, 1.3862944363729515, 1.5863556147912519, 1.7387581999321977, 1.8931822644566534, 2.0496291130716564, 2.2081000305748244, 2.368590317085447, 2.5310809278831545, 2.6955507135775, 2.8619785247780314, 3.0303432120943032, 3.1436519351149155, 3.3151977506453107, 3.488624043916916, 3.605275838477448, 3.8410334661219636, 3.9601267662338184, 4.1402671997715785, 4.322189413895119, 4.444450323937259, 4.629295173724996, 4.8158654057239865, 4.9411936619194705, 5.130592532291775, 5.257786964633858, 5.449955219712827, 5.643750192823404, 5.773834973578106, 5.970270885278316, 6.102086969942791, 6.301075854782724, 6.434564599723707, 6.636018492253445, 6.771121253837677, 6.974952188607315};
    
    // mass values for host at each snapshot after peak mass of tree id 101 (in code units, 10^10 Msol)
    double NFW_M200_table[44] = {702.115901635043, 701.4729781667497, 715.2517953382187, 722.872269762488, 739.8411514557038, 789.4939808920136, 927.495784663788, 1028.2156859522622, 1072.71053110353, 1104.627124391054, 1134.6231916149986, 1162.2848938727118, 1197.2408108842171, 1218.832335290495, 1239.205949736585, 1238.2500566561946, 1232.9507021908216, 1234.001112364371, 1240.4682800274948, 1247.033749071588, 1246.5176442959755, 1237.0483650723354, 1241.583834012631, 1234.386479006357, 1217.2514040068183, 1214.9913732654263, 1213.430264441707, 1216.7443995423678, 1214.2323532532478, 1210.2584005422848, 1202.5014214505832, 1192.2357298471177, 1181.8919197310026, 1175.4931395547312, 1167.4139553866346, 1165.3332278057646, 1160.932551593178, 1156.2278709265206, 1159.2410650441948, 1175.5266575324772, 1179.8593967686652, 1181.8611048159785, 1181.2293089558884, 1190.4110724080583};

    // host concentration values for tree id 101
    double NFW_C_table[44] = {4.280040537778925, 4.316340408537763, 4.357024437053903, 4.389391027562051, 4.428962293776431, 4.437905255802892, 4.416392499797274, 4.407100437777999, 4.423549267382294, 4.445444373688645, 4.47993915434174, 4.503859315801812, 4.524994146799027, 4.5515244573084646, 4.578598317184521, 4.614213096172665, 4.651782599738929, 4.6868773525861, 4.719724706700444, 4.752512146660989, 4.776490892336079, 4.816394063493522, 4.850052997484545, 4.87717668954275, 4.932981855787691, 4.95792833624921, 4.994407963454633, 5.028335855603767, 5.053255673023753, 5.090661606773518, 5.129955339900054, 5.158866295198239, 5.199527660822387, 5.226332605618717, 5.265618879878386, 5.301350761386425, 5.3268440248583815, 5.363849176525055, 5.384707355607285, 5.408630930246112, 5.428299816339302, 5.4602268488889925, 5.482525579810147, 5.509468843150602};
    
    /* before the particle loop, interpolate the relevant quantities to the simulation time */
    double t=All.Time, dt=0; int i, i0=0, i1=0, k;
    if(t<=t_table[0])
    {
        i0=i1=0;
    } else if(t>=t_table[n_table-1]) {
        i0=i1=n_table-1;
    } else {
        for(k=1;k<n_table;k++) {if(t_table[k] > t) {i1=k; break;}}
        i0=i1-1; dt=(t - t_table[i0])/(t_table[i1]-t_table[i0]);
    }
    double NFW_M200 = NFW_M200_table[i0] + dt * (NFW_M200_table[i1]-NFW_M200_table[i0]);
    double NFW_C = NFW_C_table[i0] + dt * (NFW_C_table[i1]-NFW_C_table[i0]);
...
/* Triaxial NFW-like analytic acceleration
   - axis_ratios: qa = b/a, qc = c/a (intermediate/major, minor/major)
   - we set a_scale = Rs, b_scale = qa*Rs, c_scale = qc*Rs
   - M200 is the spherical M200 (mass within R200); we use the usual NFW enclosed-mass formula with normalization cfac.
*/
void GravAccel_TriaxialNFW()
{
    /* --- user parameters: tune these in code units --- */
    double NFW_M200 = 1000;    /* mass inside R200 in code mass units (same meaning as in your StaticNFW) */
    double NFW_C   = 4;      /* concentration c = R200 / Rs */
    double qa = 0.95;           /* b/a (intermediate/major axis ratio) */
    double qc = 0.6;           /* c/a (minor/major axis ratio) */
    double G = All.G;

    /* compute Rs and normalization exactly as in StaticNFW */
    double R200 = pow(NFW_M200 * G / (100.0 * All.Hubble_H0_CodeUnits * All.Hubble_H0_CodeUnits), 1.0/3.0);
    double Rs = R200 / NFW_C;
    double cfac = log(1.0 + NFW_C) - NFW_C / (1.0 + NFW_C); /* normalization */

    /* derived axis scale lengths (same units as P[i].Pos) */
    double a_scale = Rs;
    double b_scale = qa * Rs;
    double c_scale = qc * Rs;

    int i, k;
    for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
    {
        double dp[3];
        for(k = 0; k < 3; k++) dp[k] = P[i].Pos[k];

        /* optional anchoring/periodic shift (same as other routines) */
#ifdef GRAVITY_ANALYTIC_ANCHOR_TO_PARTICLE
        for(k=0;k<3;k++) { dp[k] = -P[i].min_xyz_to_bh[k]; }
#elif defined(BOX_PERIODIC)
        dp[0] -= boxHalf_X; dp[1] -= boxHalf_Y; dp[2] -= boxHalf_Z;
        ...
        /* scaled coordinates (dimensionless) and ellipsoidal radius m */
        double qx = dp[0] / a_scale;
        double qy = dp[1] / b_scale;
        double qz = dp[2] / c_scale;
        double m2 = qx*qx + qy*qy + qz*qz;
        double m  = sqrt(m2);

        /* if at the exact center, skip (no accel) */
        if(m <= 0.0) { continue; }

        /* physical ellipsoidal radius where we evaluate NFW (r_phys = m * Rs) */
        double r_phys = m * Rs;   /* same units as dp (code length units) */

        /* compute enclosed-mass factor f(m) = ln(1+m) - m/(1+m)
           use small-m Taylor expansion for numerical stability */
        double fnum;
        if(m <= 0.04) {
            /* Taylor: f(m)/m^2 ≈ 0.5 - (2/3) m + (3/4) m^2 -> but we need fnum = m^2*(that) */
            double t = 0.5 - 2.0*m/3.0 + 0.75*m*m;
            fnum = t * (m*m); /* fnum ≈ (ln(1+m)-m/(1+m)) */
        } else {
            fnum = log(1.0 + m) - m / (1.0 + m);
        }

        /* enclosed mass M(<r_phys) = M200 * fnum / cfac */
        double Menc = NFW_M200 * (fnum / cfac);

        /* acceleration magnitude factor: -G * Menc / r_phys^3 */
        double prefac = -G * Menc / (r_phys * r_phys * r_phys);

        /* acceleration components: note we multiply by original coordinate dp[k].
           This gives the correct vector and reduces to spherical case when qa=qc=1. */
        P[i].GravAccel[0] += prefac * dp[0];
        P[i].GravAccel[1] += prefac * dp[1];
        P[i].GravAccel[2] += prefac * dp[2];
    }
}
